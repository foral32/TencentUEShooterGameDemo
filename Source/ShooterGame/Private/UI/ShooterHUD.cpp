...
void AShooterHUD::DrawHUD()
{
	...
	if (MatchState == EShooterMatchState::Playing)
	{
		...
		//++[civ][Gao Jiacheng] 补充if条件使得操纵炮台不会弹出复活提示,即当前操纵为pawn不为空且其instigator为玩家
		else if (GetOwningPawn() && GetOwningPawn()->GetInstigator() && Cast<AShooterCharacter>(GetOwningPawn()->GetInstigator())) {
